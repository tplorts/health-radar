...
function placeMalariaMarkers( pMap ) {
	var countryData = updateCountryInfo();
	
	$.get('WHS3_48.xml', function(malariaXML) {
		updateMalariaData( pMap, countryData, malariaXML );
	});
}
...
function updateMalariaData( pMap, pCountryData, malariaXML ) {
	...
	for( i = 0; i < caseElements.length; i++ ) {
		...
		placeCircleMarker( pMap, country.latitude, country.longitude, Math.log(thisQuantity/10) );
	}
